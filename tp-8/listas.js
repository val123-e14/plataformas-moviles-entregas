...
function crearListaDeNumeros(inicio, fin) {
    var nuevaListaDeNumeros = [];
    for(var i=0; i<=fin; i++){
        if(i >= inicio){
            nuevaListaDeNumeros[i] = i;
        }
    }
...
function encontrarPalabraMasCorta(listaDePalabras) {
    ...
    palabra.length < palabraMasCorta.length ? palabra : palabraMasCorta
    );
}
...
function contarAprobados(listaDeNotas) {
    const cantidadDeAprobados = listaDeNotas.filter(nota => nota >= 7);
